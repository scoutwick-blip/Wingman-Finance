...
export const SetupWizard: React.FC<SetupWizardProps> = ({ onComplete }) => {
  ...
  return (
    <div className="fixed inset-0 bg-slate-900 z-[100] flex items-center justify-center p-4">
      <div className="max-w-lg w-full bg-white rounded-[2rem] shadow-2xl p-8 md:p-12 space-y-10 relative overflow-hidden border-t-8 border-indigo-600">
          ...
          {step === 1 && (
            <div className="space-y-6">
              <input 
                ...
                className="w-full bg-slate-50 border-2 border-slate-100 rounded-xl px-6 py-4 text-sm font-bold outline-none focus:border-indigo-600 transition-all uppercase"
              />
            </div>
          )}
        ...
        <div className="flex gap-4">
          {step > 1 && (
            <button onClick={() => setStep(step - 1)} className="flex-1 bg-slate-100 text-slate-600 py-4 rounded-xl font-black uppercase text-[10px] tracking-widest">
              Back
            </button>
          )}
          <button 
            ...
            onClick={handleNext}
            className="flex-[2] bg-slate-900 text-white py-4 rounded-xl font-black uppercase text-[10px] tracking-[0.2em] shadow-xl hover:bg-slate-800 transition-all"
          >
